...
function getNextFriday(date) {
  ...
  nextFriday.setDate(nextFriday.getDate() + count);
...
function isDateInPeriod(date, period) {
  ...
  return startDate <= dateAsDate && dateAsDate <= endDate;
}
...
function formatDate(date) {
  ...
  let hours = dateAsDate.getUTCHours();
  const minutes = String(dateAsDate.getUTCMinutes()).padStart(2, '0');
  const seconds = String(dateAsDate.getUTCSeconds()).padStart(2, '0');
...
function getWeekNumberByDate(date) {
  const startDate = new Date(Date.UTC(date.getUTCFullYear(), 0, 1));
  ...
  if (date < startDate) {
    return getWeekNumberByDate(
      new Date(Date.UTC(date.getUTCFullYear() - 1, 11, 31))
    );
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const curDate = new Date(period.start.split('-').reverse().join('-'));
  ...
  while (curDate <= endDate) {
    if (!isWeekend && countOfWorkingDays < countWorkDays) {
      countOfWorkingDays += 1;
      res.push(
        `${curDate.getDate().toString().padStart(2, '0')}-${(curDate.getMonth() + 1).toString().padStart(2, '0')}-${curDate.getFullYear()}`
      );
    } else {
      isWeekend = true;
      countOfWorkingDays = 0;
      countOfWeekends += 1;
      if (countOfWeekends >= countOffDays) {
        countOfWeekends = 0;
        isWeekend = false;
      }
    }
